...
function ErrorAnalysis(results_path::String, 
    ...
    return ErrorAnalysis(N_c, N_e, V_err, w_err, volq_to_err, xyzq, J_q,
        total_volume, results_path)
end
...
function analyze(analysis::ErrorAnalysis{d}, sol::Array{Float64,3},
    ...
    (; N_c, N_e, J_q, w_err, V_err, volq_to_err, xyzq, 
        total_volume, results_path) = analysis 
    ...
    @inbounds @views for k in 1:N_e
        ...
        for e in 1:N_c
            ...
            error[e] += dot(error_nodal, 
                (w_err .* volq_to_err*J_q[:,k]) .* error_nodal)
        end
    end
