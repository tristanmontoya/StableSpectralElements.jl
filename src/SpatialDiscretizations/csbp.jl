...
Construct a classical one-dimensional finite difference SBP operator of degree `p`.

The number of nodes is minimal for the boundary stencil, plus one interior node.
...
- B. Strand (1994)
  ...
  [DOI: 10.1006/jcph.1994.1005](https://doi.org/10.1006/jcph.1994.1005)
"""
...
function get_1d_csbp(p)
    ...
    elseif p == 3
        ...
        Pinv = vcat(43200 / 13649, 8640 / 12013, 4320 / 2711,
            4320 / 5359, 8640 / 7877, 43200 / 43801,
            ones(1, N - 12),
            43200 / 43801, 8640 / 7877, 4320 / 5359, 4320 / 2711, 8640 / 12013, 43200 /
                                                                                13649) / dx
        H = zeros(N, N)
        for i in 1:N
            H[i, i] = 1 / Pinv[i]
    ...
    elseif p == 4
        ...
        dx = L / (N - 1)
        x = collect(range(-1, 1, length = N))
        Pinv = vcat(1498139 / 5080320, 1107307 / 725760, 20761 / 80640, 1304999 / 725760,
            299527 / 725760, 103097 / 80640, 670091 / 725760, 5127739 / 5080320,
            ones(1, N - 16),
            5127739 / 5080320, 670091 / 725760, 103097 / 80640, 299527 / 725760, 1304999 /
                                                                                 725760,
            20761 / 80640, 1107307 / 725760, 1498139 / 5080320) / dx
        H = zeros(N, N)
        for i in 1:N
            H[i, i] = 1 / Pinv[i]
        end
