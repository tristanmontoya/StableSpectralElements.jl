...
Construct the one-dimensional diagonal-norm SBP operator of degree `p` with the
optimized coefficients of K. Mattsson, M. Almquist, and M. H. Carpenter (2014).

Returns `(D, Q, H, x)`, where `D` is the differentiation matrix, `Q = H * D`,
`H` is the norm matrix, and `x` contains the nodal coordinates on `[-1, 1]`.

# References
- K. Mattsson, M. Almquist, M. H. Carpenter (2014)
  Optimal diagonal-norm SBP operators.
  [DOI: 10.1016/j.jcp.2013.12.041](https://doi.org/10.1016/j.jcp.2013.12.041)
"""
...
"""
    get_1d_optimal(p::Int, L::Float64 = 1.0)

Construct the boundary optimized one-dimensional diagonal-norm SBP operator of degree
`p` with coefficients optimized for small solution error.

Returns `(D, Q, H, x)`, where `D` is the differentiation matrix, `Q = H * D`,
`H` is the norm matrix, and `x` contains the nodal coordinates on `[-1, 1]`.

# References
- K. Mattsson, M. Almquist, E. van der Weide (2018)
  Boundary optimized diagonal-norm SBP operators.
  [DOI: 10.1016/j.jcp.2018.06.010](https://doi.org/10.1016/j.jcp.2018.06.010)
"""
function get_1d_optimal(p::Int, L::Float64 = 1.0)
    if p == 2
        ...
        Qmat = [-5.0000000000000e-01 6.5605279837843e-01 -1.9875859409017e-01 4.2705795711740e-02 0.0 0.0
    ...
    elseif p == 3
        ...
        Qmat = [-5.0000000000000e-01 6.6042071945824e-01 -2.2104152954203e-01 7.6243679810093e-02 -1.7298206716724e-02 1.6753369904210e-03 0.0 0.0 0.0
    ...
    elseif p == 6
        ...
        H *= 2 * h # note the factor of 2 is to define the operator between -1 and 1 since the original operators are defined between 0 and 1
        Q = zeros(N, N)
        d = [1 / 5544, -1 / 385, 1 / 56, -5 / 63, 15 / 56, -6 / 7, 0,
        ...
        Qmat = [Q0_0 Q0_1 Q0_2 Q0_3 Q0_4 Q0_5 Q0_6 Q0_7 Q0_8 Q0_9 Q0_10 Q0_11 Q0_12 Q0_13 Q0_14 Q0_15 Q0_16 Q0_17;
...
"""
    get_1d_minimal(p::Int, L::Float64 = 1.0)

Construct the boundary optimized one-dimensional diagonal-norm SBP operator of degree
`p` with coefficients optimized for a small number of boundary nodes.
...
function get_1d_minimal(p::Int, L::Float64 = 1.0)
    ...
    elseif p == 6
        ...
        Qmat = [Q0_0 Q0_1 Q0_2 Q0_3 Q0_4 Q0_5 Q0_6 Q0_7 Q0_8 Q0_9 Q0_10 Q0_11 Q0_12 Q0_13 Q0_14 Q0_15;
